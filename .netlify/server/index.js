...
${
	components[1]
		? `${(0, import_chunks.v)(components[0] || import_chunks.m, 'svelte:component').$$render(
				$$result,
				{ data: data_0, errors },
				{},
				{
					default: () => {
						return `${(0, import_chunks.v)(
							components[1] || import_chunks.m,
							'svelte:component'
						).$$render($$result, { data: data_1, errors }, {}, {})}`;
					}
				}
		  )}`
		: `${(0, import_chunks.v)(components[0] || import_chunks.m, 'svelte:component').$$render(
...
function get_option(nodes, option) {
	return nodes.reduce((value, node) => {
		...
		return (_b2 =
			(_a2 = (_a = node == null ? void 0 : node.shared) == null ? void 0 : _a[option]) != null
				? _a2
...
function serialize_data(fetched, prerendering = false) {
	...
	if (!prerendering && fetched.method === 'GET') {
		...
		if (cache_control) {
			const match =
				(_a = /s-maxage=(\d+)/g.exec(cache_control)) != null
					? _a
...
async function render_response({
	...
	if (validation_errors) {
		try {
			...
			const error3 = e;
			if (error3.path) throw new Error(`${error3.message} (errors.${error3.path})`);
			throw error3;
		}
	}
	const init_app = `
		...
		if (is_asset || is_asset_html) {
			...
			if (opts.credentials !== 'omit') {
				...
				for (const cookie3 of cookies) {
					if (!domain_matches(event.url.hostname, cookie3.domain)) continue;
					if (!path_matches(resolved, cookie3.path)) continue;
...
async function respond(request, options, state) {
	...
	if ((route == null ? void 0 : route.page) && !is_data_request) {
		...
		if (
			...
			!((_e = state.prerendering) == null ? void 0 : _e.fallback)
		) {
			return new Response(void 0, {
				status: 301,
				headers: {
